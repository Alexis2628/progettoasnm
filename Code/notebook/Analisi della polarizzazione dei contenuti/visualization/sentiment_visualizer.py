...
class SentimentVisualizer:
    ...
    def visualize_sentiment_distribution(self, sentiment_by_user, cluster_labels):
        ...
        data = pd.DataFrame(
            {
                ...
                "Sentiment": [
                    sentiment_by_user[user]["average_score"]
                    for user in sentiment_by_user.keys()
                ],
                "Cluster": [
                    cluster_labels.get(user, None) for user in sentiment_by_user.keys()
                ],
            }
        )
        ...
        if data.empty:
            logging.warning(
                "Il DataFrame risultante è vuoto. Nessun grafico verrà generato."
            )
            return
        ...
        if not os.path.exists(violin_plot_path):
            ...
            gc.collect()
            print(
                f"Violin plot salvato in: {violin_plot_path} (dimensione: {os.path.getsize(violin_plot_path)} bytes)"
            )
        ...
        if not os.path.exists(bar_plot_path):
            ...
            gc.collect()
            print(
                f"Bar plot salvato in: {bar_plot_path} (dimensione: {os.path.getsize(bar_plot_path)} bytes)"
            )
    ...
    def visualize_sentiment_vs_themes_heatmap(
        ...
        df = pd.DataFrame(
            {
                ...
                "Sentiment": [
                    (
                        ...
                        if (
                            u in sentiment_scores
                            and isinstance(sentiment_scores[u], dict)
                            and "average_score" in sentiment_scores[u]
                        )
                        else None
                    )
                    for u in user_opinions.keys()
                ],
                "Cluster": [cluster_labels.get(u, None) for u in user_opinions.keys()],
            }
        )
        ...
        heatmap_data = pd.DataFrame(
            {"Terms": heatmap_words, "Correlation": heatmap_corrs}
        ).set_index("Terms")
