...
class ClusteringTFIDF:
    ...
    def _fit_vectorizer(self, texts: List[str]) -> np.ndarray:
        ...
        if self.use_lsa:
            ...
            self.svd_model = TruncatedSVD(
                n_components=self.lsa_components, random_state=42
            )
            ...
            logging.info(
                f"Modello LSA salvato in {self.lsa_file} con {self.lsa_components} componenti"
            )
            return X_reduced
    ...
    def cluster(
        ...
        if spherical:
            ...
            logging.info(
                "Dati normalizzati L2 per spherical k-means (Dhillon & Modha, 2001)"
            )
        ...
        if method == "kmeans":
            ...
            model = DBSCAN(
                ...
                metric="cosine" if spherical else "euclidean",
            )
        elif method == "hdbscan":

            metric_hdb = "euclidean"
            model = hdbscan.HDBSCAN(
                min_cluster_size=hdbscan_min_cluster_size, metric=metric_hdb
            )
            ...
            raise ValueError(
                f"Metodo {method} non riconosciuto. Scegli tra kmeans, dbscan, hdbscan, spectral, agglomerative."
            )
...
class ClusteringEmbeddings:
    ...
    def _reduce_dimensionality(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        if self.umap_model is None:
            logging.info(
                f"Applico UMAP: dim originali {embeddings.shape[1]}, ridotte a {self.umap_components}"
            )
    ...
    def cluster(
        ...
        if method == "kmeans":
            ...
            model = AgglomerativeClustering(
                n_clusters=n_clusters, affinity="cosine", linkage=agglo_linkage
            )
        else:
            raise ValueError(
                f"Metodo {method} non riconosciuto. Scegli tra kmeans, dbscan, hdbscan, spectral, agglomerative."
    ...
    def identify_polarizing_themes(
        ...
        """
        logging.info(
            f"Identificazione temi polarizzanti da embedding, ngram_range={ngram_range!r}."
        )
        ...
        for cluster_id, texts in clusters.items():
            ...
            try:
                ...
                logging.warning(
                    f"Cluster {cluster_id}: impossibile vettorizzare testi: {e}. Skip."
                )
                continue
            ...
            polarizing[cluster_id] = keywords
            logging.debug(
                f"[Embeddings] Cluster {cluster_id}, {ngram_range}: {keywords}"
            )
